...
import type {
  ChangeEstablishmentStatusResponse,
  ...
  UpdateMyEstablishmentResponse,
  UploadEstablishmentAvatarResponse,
  UploadEstablishmentImagesResponse,
  VerifyEstablishmentResponse,
...
export const establishmentService = {
  ...
  getBySlug: (slug: string): GetEstablishmentByIdResponse =>
    api.get(`/establishment/${slug}`),
  getMine: (): GetMyEstablishmentsResponse => api.get("/establishment/mine"),
  createMine: (payload: CreateEstablishmentDto): CreateEstablishmentResponse =>
    api.post("/establishment/mine", payload),
  updateMine: (
    id: string,
    payload: EditEstablishmentDto,
  ): UpdateMyEstablishmentResponse => api.put(`/establishment/${id}`, payload),
  deleteMine: (id: string): DeleteMyEstablishmentResponse =>
    api.delete(`/establishment/${id}`),
  uploadImages: (
    id: string,
    formData: FormData,
  ): UploadEstablishmentImagesResponse =>
    api.postForm(`/establishment/${id}/images`, formData),
  uploadAvatar: (
    id: string,
    formData: FormData,
  ): UploadEstablishmentAvatarResponse =>
    api.postForm(`/establishment/${id}/avatar`, formData),
  verify: (id: string, verified: boolean): VerifyEstablishmentResponse =>
    api.patch(`/establishment/${id}/verify`, { verified }),
  deleteImage: (
    id: string,
    imageId: string,
  ): DeleteEstablishmentImageResponse =>
    api.delete(`/establishment/${id}/image/${imageId}`),
  changeStatus: (
    id: string,
    status: EstablishmentStatus,
  ): ChangeEstablishmentStatusResponse =>
    api.patch(`/establishment/${id}/status`, { status }),
};
