...
export const useEstablishmentApi = () => {
    ...
    const useGetEstablishment = (id: string, options?: { enabled?: boolean }) => {
        return useQuery({
            ...
            queryFn: () => establishmentService.getById(id).then(res => res?.data),
    ...
    const useGetEstablishments = (queryParams: string) => {
        return useQuery({
            ...
            queryFn: () =>
                establishmentService.getEstablishments(queryParams).then((res) => res?.data),
        });
    };
    ...
    const createEstablishment = useMutation({
        mutationFn: (data: CreateEstablishmentDto) => establishmentService.createMine(data).then(res => res?.data),
        onSuccess: () => {
    ...
    const updateEstablishment = useMutation({
        mutationFn: (data: EditEstablishmentDto) => establishmentService.updateMine(data.id, data).then(res => res?.data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["establishments", "me"] });
    ...
    const updateEstablishmentAvatar = useMutation({
        mutationFn: ({ id, formData }: { id: string, formData: FormData }) => establishmentService.uploadAvatar(id, formData).then(res => res?.data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["establishments", "me"] });
        },
        onError: (error) => {
    ...
    const updateEstablishmentImages = useMutation({
        mutationFn: ({ id, formData }: { id: string, formData: FormData }) => establishmentService.uploadImages(id, formData).then(res => res?.data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["establishments", "me"] });
        },
        onError: (error) => {
    ...
    const deleteEstablishment = useMutation({
        mutationFn: (id: string) => establishmentService.deleteMine(id).then(res => res?.data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["establishments", "me"] });
    ...
    const deleteEstablishmentImage = useMutation({
        mutationFn: ({ id, imageId }: { id: string, imageId: string }) => establishmentService.deleteImage(id, imageId).then(res => res?.data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["establishments", "me"] });
    ...
    const changeEstablishmentStatus = useMutation({
        mutationFn: ({ id, status }: { id: string, status: EstablishmentStatus }) => establishmentService.changeStatus(id, status).then(res => res?.data),
        onSuccess: ({ message }) => {
